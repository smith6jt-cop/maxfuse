...
from . import match_utils, graph, utils, spatial_utils
...
class Fusor:
    ...
    def __init__(
            ...
            locations2=None, spatial_n_neighbors=15,
            use_spatial_context=False, spatial_context_weight=0.3
    ):
        """
        ...
        :param locations2: Spatial coordinates for the second dataset (CODEX).
            Shape (n_samples2, 2) with X, Y coordinates.
        :type locations2: Union[np.ndarray, None]
        :param spatial_n_neighbors: Number of spatial neighbors for neighborhood
            composition. Only used if use_spatial_context=True.
        :type spatial_n_neighbors: int
        :param use_spatial_context: Whether to augment features with spatial
            neighborhood composition.
        :type use_spatial_context: bool
        :param spatial_context_weight: Weight on original features vs
            neighborhood composition. Only used if use_spatial_context=True.
        :type spatial_context_weight: float
        """
        # input
        ...
        # region-based priors (set via set_region_priors method)
        self._rna_labels = None
        self._spatial_regions2 = None
        self._celltype_to_region_weights = None
        self._wt_on_base_dist = 0.0

        # post-hoc filter settings (set via set_posthoc_filter method)
        self._posthoc_filter_func = None
        self._posthoc_rna_labels = None
        self._posthoc_spatial_regions = None
    ...
    def set_region_priors(
            self,
            rna_labels,
            spatial_regions=None,
            celltype_to_region_weights=None,
            wt_on_prior=0.3,
            auto_detect_regions=False,
            marker_expression=None,
            marker_names=None,
            marker_to_region=None,
            verbose=True
    ):
        """
        Configure region-based prior weights for matching.

        This method sets up priors that favor biologically plausible matches
        (e.g., B cells matching to B follicle regions) over unlikely ones
        (e.g., B cells matching to red pulp regions).

        Parameters
        ----------
        rna_labels : np.ndarray of shape (n_rna_cells,)
            Cell type labels for RNA cells (arr1).
        spatial_regions : np.ndarray of shape (n_spatial_cells,) or None
            Region labels for spatial cells (arr2). If None and
            auto_detect_regions=True, regions will be detected automatically.
        celltype_to_region_weights : dict or None
            ...
            If None, uses uniform weights (no prior effect).
        wt_on_prior : float, default=0.3
            Weight on prior distance (0-1).
            Final dist = (1-wt)*embedding_dist + wt*prior_dist
        auto_detect_regions : bool, default=False
            If True and spatial_regions is None, automatically detect regions
            using marker expression and spatial clustering.
        marker_expression : np.ndarray or None
            Required if auto_detect_regions=True. Protein marker expression
            matrix of shape (n_spatial_cells, n_markers).
        marker_names : list of str or None
            Required if auto_detect_regions=True. Names of markers.
        marker_to_region : dict or None
            Required if auto_detect_regions=True. Maps marker names to region
            names, e.g., {'CD20': 'B_follicle', 'CD3e': 'T_zone'}.
        ...
        Notes
        -----
        Must be called after split_into_batches() but before find_initial_pivots().
        """
        # Store RNA labels
        self._rna_labels = np.array(rna_labels)

        # Handle region detection/assignment
        if spatial_regions is not None:
            self._spatial_regions2 = np.array(spatial_regions)
        elif auto_detect_regions:
            if self.locations2 is None:
                raise ValueError(
                    "locations2 must be provided at initialization to auto-detect regions"
                )
            if marker_expression is None or marker_names is None or marker_to_region is None:
                raise ValueError(
                    "marker_expression, marker_names, and marker_to_region are required "
                    "for auto_detect_regions=True"
                )
            if verbose:
                print("Auto-detecting tissue regions from spatial neighborhoods...", flush=True)

            self._spatial_regions2, region_info = spatial_utils.detect_tissue_regions(
            ...
            if verbose:
                print(f"  Detected {region_info['n_regions']} regions:", flush=True)
                for region, count in region_info['region_counts'].items():
                    print(f"    {region}: {count} cells", flush=True)
        else:
            raise ValueError(
                "Either spatial_regions must be provided or auto_detect_regions=True"
            )

        # Store prior configuration
        self._celltype_to_region_weights = celltype_to_region_weights
        self._wt_on_base_dist = wt_on_prior

        if verbose:
            unique_types = np.unique(self._rna_labels)
            unique_regions = np.unique(self._spatial_regions2)
            print(f"Configured region priors:", flush=True)
            print(f"  RNA cell types: {len(unique_types)}", flush=True)
            print(f"  Spatial regions: {len(unique_regions)}", flush=True)
            print(f"  Prior weight: {wt_on_prior}", flush=True)

    def _compute_batch_prior_dist(self, b1, b2):
        """
        Compute prior distance matrix for batch (b1, b2).

        Parameters
        ----------
        b1 : int
            Batch index for arr1 (RNA).
        b2 : int
            Batch index for arr2 (spatial/CODEX).

        Returns
        -------
        base_dist : np.ndarray of shape (n_batch1, n_batch2) or None
            Prior distance matrix for this batch, or None if no priors configured.
        """
        if self._celltype_to_region_weights is None:
            return None

        # Get batch indices
        indices1 = self._batch_to_indices1[b1]
        indices2 = self._batch_to_indices2[b2]

        # Get labels/regions for this batch
        rna_labels_batch = self._rna_labels[indices1]
        spatial_regions_batch = self._spatial_regions2[indices2]

        # Handle metacells - use majority voting for metacell labels
        if self.metacell_size > 1 and self._metacell_labels1 is not None:
            metacell_labels = self._metacell_labels1[b1]
            unique_metacells = np.unique(metacell_labels)
            metacell_rna_labels = []
            for mc in unique_metacells:
                mc_mask = metacell_labels == mc
                mc_cell_types = rna_labels_batch[mc_mask]
                # Majority voting
                unique, counts = np.unique(mc_cell_types, return_counts=True)
                metacell_rna_labels.append(unique[np.argmax(counts)])
            rna_labels_batch = np.array(metacell_rna_labels)

        return spatial_utils.compute_region_celltype_prior(
            celltype_to_region_weights=self._celltype_to_region_weights,
            rna_labels=rna_labels_batch,
            spatial_regions=spatial_regions_batch
        )

    def set_posthoc_filter(
            self,
            filter_func=None,
            rna_labels=None,
            spatial_regions=None,
        ...
        """
        Configure post-hoc filtering of biologically implausible matches.

        After matching is complete, this filter can remove matches that
        violate biological expectations (e.g., B cells matched to red pulp).

        Parameters
        ----------
        filter_func : callable or None
            Custom filter function with signature:
            filter_func(idx1, idx2, score, rna_labels, spatial_regions) -> bool
            Returns True to KEEP the match, False to remove it.
        rna_labels : np.ndarray or None
            Cell type labels for RNA cells.
        spatial_regions : np.ndarray or None
            Region labels for spatial cells.
        forbidden_pairs : list of tuples or None
            List of (celltype, region) pairs that should be filtered out.
            E.g., [('B_cell', 'Red_pulp'), ('Macrophage', 'B_follicle')]
        ...
        if filter_func is not None:
            self._posthoc_filter_func = filter_func
        elif forbidden_pairs is not None:
            forbidden_set = set(forbidden_pairs)

            def default_filter(idx1, idx2, score, rna_labels, spatial_regions):
                pair = (rna_labels[idx1], spatial_regions[idx2])
                return pair not in forbidden_set

            self._posthoc_filter_func = default_filter
        else:
            self._posthoc_filter_func = None

        self._posthoc_rna_labels = rna_labels
        self._posthoc_spatial_regions = spatial_regions

    def _apply_posthoc_filter(self, matching):
        """
        Apply post-hoc filter to a matching result.

        Parameters
        ----------
        matching : list of [rows, cols, vals]
            Matching result.

        Returns
        -------
        filtered_matching : list of [rows, cols, vals]
            Filtered matching result.
        n_removed : int
            Number of matches removed.
        """
        if self._posthoc_filter_func is None:
            return matching, 0
        ...
        for i in range(len(matching[0])):
            idx1, idx2, score = matching[0][i], matching[1][i], matching[2][i]
            if self._posthoc_filter_func(
                    idx1, idx2, score,
                    self._posthoc_rna_labels,
                    self._posthoc_spatial_regions
            ):
                filtered[0].append(idx1)
                filtered[1].append(idx2)
                filtered[2].append(score)
            else:
                n_removed += 1
    ...
    def find_initial_pivots(
        ...
        svd_runs: int, default=1
            Perform multiple runs of SVD and the one with lowest Frobenious reconstruction error is selected.
        use_region_priors: bool, default=True
            Whether to use region-based prior weights if configured via set_region_priors().
        verbose: bool, default=True
            Whether to print the progress.
        ...
        for b1, b2 in self._batch1_to_batch2:
            ...
            # Compute prior distance matrix if configured
            base_dist = None
            wt_on_base_dist = 0.0
            if use_region_priors and self._celltype_to_region_weights is not None:
                base_dist = self._compute_batch_prior_dist(b1, b2)
                wt_on_base_dist = self._wt_on_base_dist
                if verbose and base_dist is not None:
                    print(f'  Using region priors with weight {wt_on_base_dist}', flush=True)
            ...
            self._init_matching.append(
                match_utils.get_initial_matching(
                    ...
                    base_dist=base_dist,
                    wt_on_base_dist=wt_on_base_dist,
                    verbose=False
                )
            )
...
#         return fig, ax, bkps[::-1]
    ...
    def get_matching(self, order=None, target='pivot', apply_posthoc_filter=True,
                     verbose=False):
        """
        ...
        apply_posthoc_filter: bool, default=True
            If True and a post-hoc filter is configured via set_posthoc_filter(),
            apply it to filter out biologically implausible matches.
        ...
        res = match_utils.address_matching_redundancy(matching=res, order=order)

        # Apply post-hoc filter if configured
        if apply_posthoc_filter and self._posthoc_filter_func is not None:
            res, n_removed = self._apply_posthoc_filter(res)
            if verbose:
                total = len(res[0]) + n_removed
                print(f'Post-hoc filter removed {n_removed}/{total} matches '
                      f'({100*n_removed/total:.1f}%)', flush=True)
