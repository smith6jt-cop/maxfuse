...
def get_initial_matching(
    ...
    base_dist: None or np.ndarray of shape (n_samples1, n_samples2)
        Prior distance matrix for region-aware matching. If provided, the final distance
        will be interpolated: (1-wt_on_base_dist) * embed_dist + wt_on_base_dist * base_dist
    wt_on_base_dist: float, default=0.0
        Weight on the prior distance matrix. 0 = ignore priors, 1 = use only priors.
    verbose: bool, default=True
        Whether to print the progress.
...
def get_refined_matching_one_iter(
    ...
    base_dist: None or np.ndarray of shape (n_samples1, n_samples2)
        Prior distance matrix for region-aware matching.
    wt_on_base_dist: float, default=0.0
        Weight on the prior distance matrix.
    verbose: bool, default=True
        Whether to print the progress.
...
def get_refined_matching(
    ...
    for it in range(n_iters):
        ...
        cca_matching = get_refined_matching_one_iter(
            ...
            base_dist=base_dist, wt_on_base_dist=wt_on_base_dist,
            verbose=verbose
        )
