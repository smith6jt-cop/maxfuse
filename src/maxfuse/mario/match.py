...
class Mario(object):
    ...
    def _align_modalities(self, matching, overlap_only=False):
        ...
        overlap_only : bool, default=False
            If True, only use overlapping features for alignment.
        ...
        if overlap_only:
            df1_use = self.df1[self.ovlp_features]
            df2_use = self.df2[self.ovlp_features]
        else:
            df1_use = self.df1
            df2_use = self.df2
        ...
        for ii in range(self.n1):
            ...
            X.append(df1_use.iloc[ii, :])
            if len(matching[ii]) == 1:
                Y.append(df2_use.iloc[matching[ii][0]])
            else:
                Y.append(df2_use.iloc[matching[ii]].mean(axis=0))
    ...
    def fit_cca(self, matching, n_components=20, max_iter=3000, overlap_only=False):
        ...
        overlap_only : bool, default=False
            If True, only use overlapping features for CCA.
            it holds that the i-th row in the first dataset is matched to the res[i]-th row in the second dataset.
        n_components : int
        ...
        if isinstance(matching, str):
            X, Y = self._align_modalities(self.matching[matching], overlap_only=overlap_only)
        else:
            X, Y = self._align_modalities(matching, overlap_only=overlap_only)
    ...
    def _matchable(self, n_sim=20, top_k=10, flip_prob=0.3, subsample_prop=1, verbose=True):
        ...
        match_subsampled = Mario(df1_subsampled, df2_subsampled, normalization=False)
        ...
        # Use overlap_only=True for overlap matching to avoid CCA overfitting when features >> samples
        cancor_ovlp_obs = np.mean(match_subsampled.fit_cca(match_subsampled.matching['ovlp'], n_components=top_k, overlap_only=True)[0])
        cancor_all_obs = np.mean(match_subsampled.fit_cca(match_subsampled.matching['all'], n_components=top_k, overlap_only=True)[0])
        ...
        for ii in range(n_sim):
            ...
            while trial_idx < max_trial:
                ...
                try:
                    ...
                    # calculate the median/mean of top_k cancors (use overlap features only)
                    cancor_ovlp_list.append(np.mean(
                        match_flipped.fit_cca(match_flipped.matching['ovlp'], n_components=top_k, overlap_only=True)[0]
                    ))
                    ...
                    cancor_all_list.append(np.mean(
                        match_flipped.fit_cca(match_flipped.matching['all'], n_components=top_k, overlap_only=True)[0]
                    ))
